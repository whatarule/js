...
// Free
  ...
  Free.prototype.chain = function ( g ) {
    return this.cata ( {
      Impure: function ( x, f ) {
        return Impure ( x, kleisli_comp ( f, g ) )  ;
      } ,
      Pure: function ( x ) {
        return g ( x )  ;
      } ,
  ...
  */
  ...
  const maybeToTask = function ( Maybe ) {
    return Maybe.cata ( {
      Just: Task.of ,
  ...
  const ioToTask = function ( IO ) {
    return new Task ( function ( rej, res ) {
      return res ( IO.f() ) ;
    } )  ;
  }  ;
  // eitherToTask :: Either a -> Task a
  const eitherToTask = function ( Either ) {
    return Either.cata ( {
      Right: Task.of  ,
      Left: Task.of  ,
    } ) ;
...
// Either via Free
  const Either = daggy.taggedSum ( { Right: [ 'x_right' ], Left: [ 'err_left' ] } ) ;
...
// interpreter
  ...
  Free.prototype.foldMap_maybe = function ( ) {
    this.foldMap ( interpret, Task.of ).fork ( console.error, console.log ) ;
  } ;
